--[[
    ...
    ✅ Multiple fallback methods for request
...
-- FIXED: Better request function with multiple methods
local function makeRequest(url)
    print("[API] Requesting:", url)
    
    -- Method 1: Try request function
    if request then
        ...
        if success and result then
            print("[API] request() method worked!")
    ...
    if http_request then
        ...
        if success and result then
            print("[API] http_request() method worked!")
            if result.Success then
                return result.Body
            elseif result.Body then
                return result.Body
            ...
            print("[API] http_request() failed:", result)
        end
    end
    
    -- Method 3: Try syn.request (Synapse)
    if syn and syn.request then
        ...
        if success and result then
            print("[API] syn.request() method worked!")
            if result.Success then
                return result.Body
            elseif result.Body then
                return result.Body
            ...
            print("[API] syn.request() failed:", result)
        end
    end
    
    -- Method 4: Try HttpGet (limited but might work)
    local success, result = pcall(function()
        return game:HttpGet(url, true)
    end)
    
    if success and result and result ~= "" then
        print("[API] HttpGet() method worked!")
        return result
    else
        print("[API] HttpGet() failed:", result)
    end
    
    print("[API] ❌ All request methods failed!")
    return nil
end

local function GetEmoteDataFromWeb()
    print("[API] Starting fetch...")
    print("[API] Current sort:", CURRENT_SORT_OPTION)
    print("[API] Search text:", CURRENT_SEARCH_TEXT)
    ...
    local responseBody = makeRequest(final_url)
    ...
    print("[API] Got response, parsing JSON...")
    
    local data_table
    local success, err = pcall(function()
        data_table = HttpService:JSONDecode(responseBody)
    end)
    
    if not success then
        warn("[API] ❌ JSON parse failed:", err)
        print("[API] Response preview:", string.sub(responseBody, 1, 200))
        return false
    end
    
    if not data_table or not data_table.data then
        warn("[API] ❌ Invalid data structure")
        return false
    end
    
    print("[API] ✅ Got", #data_table.data, "emotes")
    ...
    print("[API] Next cursor:", NEXT_API_CURSOR or "none")
    print("[API] Total cached:", #ITEM_CACHE)
